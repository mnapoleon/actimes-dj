...
class SessionStatisticsCalculatorTests(BaseTestCase):
    ...
    def test_calculate_driver_statistics_matches_model_method(self):
        ...
        for driver in calculator_stats.keys():
            ...
            self.assertAlmostEqual(
                calc_driver['best_lap_time'], 
                model_driver['best_lap_time'], 
                places=3
            )
            self.assertEqual(calc_driver['lap_count'], model_driver['lap_count'])
            self.assertEqual(calc_driver['racing_lap_count'], model_driver['racing_lap_count'])
            self.assertAlmostEqual(
                calc_driver['avg_lap_time'], 
                model_driver['avg_lap_time'], 
                places=3
            )
            self.assertAlmostEqual(
                calc_driver['consistency'], 
                model_driver['consistency'], 
                places=3
            )
    ...
    def test_calculate_fastest_lap_time_matches_model_method(self):
        ...
        if calculator_fastest is None:
            ...
            self.assertAlmostEqual(calculator_fastest, model_fastest_lap.total_time, places=3)
    ...
    def test_calculate_chart_data_structure(self):
        ...
        drivers = list(self.session.laps.values_list('driver_name', flat=True).distinct())
...
class StatisticsAccuracyTests(BaseTestCase):
    ...
    def create_additional_test_data(self):
        ...
        test_laps = [
            ...
            {'lap': 1, 'driver': 'TestDriver2', 'car_index': 1, 'time': 125.0, 'sectors': [42.0, 42.0, 41.0]},
            {'lap': 2, 'driver': 'TestDriver2', 'car_index': 1, 'time': 124.0, 'sectors': [41.5, 42.0, 40.5]},
        ]
    ...
    def test_consistency_calculation_accuracy(self):
        ...
        if len(filtered_times) > 1:
            ...
            self.assertAlmostEqual(calculated_consistency, expected_consistency, places=3)
